...
int main(int argc, char *argv[]){
	...
	unsigned char hex_buffer[4000] = "";
	unsigned char hex_out[4000];
	        ...
	        if (ns_parserr(&nsMsg, ns_s_an, i, &rr) == 0) {
	            if (ns_rr_type(rr) == ns_t_txt) {
	                ...
	                while (rdata < end) {
	                    int txt_len = *rdata;
	                    rdata++; // 길이 정보 건너뛰기
	                    printf(" sizeof(txt_record_all): %d\n",  sizeof(txt_record_all));
...
// fd
	//strcat(hex_out_cert,based64_out);
	strcat(hex_out_cert,newline2);
...
static int load_dns_info2(struct DNS_info* dp, char* truncated_dnsmsg_out, char* dnsmsg, char * ztls_cert){
	...
	*/
	...
	if(PEM_read_bio_PUBKEY(bio_key, &(dp->KeyShareEntry.skey), NULL, NULL)){
