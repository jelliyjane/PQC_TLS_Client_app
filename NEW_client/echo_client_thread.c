...
int main(int argc, char *argv[]){
    ...
    //txt, tlsa initial setting
    int txt_num = TXT_NUM;
    int tlsa_num = TLSA_NUM;
    int total_txt_num = 0;
    int total_tlsa_num = 0;
    ...
    if(DNS){
			...
			// to avoid TCP retry after UDP failure
				...
				//char *temp_str = malloc(strlen(txt_record_data) + 1);
				char temp_str[3000];
				strcpy(temp_str, txt_record_data[0]);
				//printf("temp_str: %s\n", temp_str);

				char *tok1, *tok2;

				tok1 = strtok(temp_str, " ");
				total_txt_num = strtoul(tok1, NULL, 10);
			//	printf("1st txt data %s\n", tok1);

				tok2 = strtok(NULL, " ");
				total_tlsa_num = strtoul(tok2, NULL, 10);
				printf("tok1: %d, tok2: %d\n", total_txt_num, total_tlsa_num);

				if (total_txt_num > txt_num || total_tlsa_num > 10){
					printf(" active mode on\n" );
				//	for (int i = 0; i < txt_num; ++i){
				//		printf("tring to free\n");
			//			free(txt_record_data[i]);
			//		}
			//		printf("tring to free2\n");
			//		free(txt_record_data);
			//		printf("tring to free3\n");
					//txt_num = total_txt_num;
					//tlsa_num = total_tlsa_num;
				}

			//	printf("2nd txt data %s\n",tok2);
				printf("txt_num1111: %d\n", txt_num);
	...
	// fd
				...
				{
					free(txt_record_data[i]);
					txt_record_data[i] = NULL;
				}
				free(txt_record_data);
				txt_record_data = NULL;
				...
				printf("aaaaaaa\n");
			}while(txt_num != total_txt_num || tlsa_num != total_tlsa_num);
