...
static int txt_query_retry(char *argv[], int txt_num, unsigned char query_txt_buffer[], unsigned char *txt_record_data);
...
{
	struct arg_struct3* args = (struct arg_struct3 *) arguments;
	txt_query_retry(args->argv, args->txt_num, args->txt_query_buffer , args->txt_record_data);
	pthread_exit(NULL);
}
...
int main(int argc, char *argv[]){
    ...
    if(DNS){
		...
		unsigned char query_passive_buffer[5000];
		char** passive_record_data;
		passive_record_data = (char**) malloc(sizeof(char*));
		passive_record_data[0]=(char*) malloc(sizeof(char)*5000);
		...
		struct arg_struct3 txt_passive;
			...
			txt_passive.txt_record_data = passive_record_data[0];

		struct timespec query;
		clock_gettime(CLOCK_MONOTONIC, &query);
		double querytime = (query.tv_sec) + (query.tv_nsec) / 1000000000.0;
		...
		char* tmp;
		tmp = strtok(txt_passive.txt_record_data," ");
	...
	// to avoid TCP retry after UDP failure
		...
		is_start = 1;
		clock_gettime(CLOCK_MONOTONIC, &begin);
		starting = (begin.tv_sec) + (begin.tv_nsec) / 1000000000.0;	
	...
	clock_gettime(CLOCK_MONOTONIC, &query);
	ending = (query.tv_sec) + (query.tv_nsec) / 1000000000.0;	

	totalquerytime = ending - querytime -1;
...
//	printf("ztls_cert: \n%s\n", ztls_cert);
	...
	strcat(txt_record_all, passive_txt);
	for (int i = 1; i<txt_num; i++){
	...
	*/
	...
	//printf("txt_record_all:\n%s\n\n",txt_record_all);
    	...
    	if(dns_info.CertVerifyEntry.signature_algorithms == 65184)     //rsa pss rsae sha256 0x0804
		{
...
static int txt_query_retry(char *argv[], int txt_num, unsigned char query_txt_buffer[], unsigned char *txt_record_all) {
    int response = -1;
    ns_type type;
	type= ns_t_txt;
    ns_msg nsMsg;
			...
			if (ns_parserr(&nsMsg, ns_s_an, i, &rr) == 0) {
			    if (ns_rr_type(rr) == ns_t_txt) {
			        ...
			        while (rdata < end) {
			            ...
			            if (offset + txt_len < 7000 - 1) { // 버퍼 크기를 초과하지 않도록 확인
...
static int tlsa_query(char *argv[], int tlsa_num, unsigned char query_buffer[], int buffer_size, unsigned char ** tlsa_record_all, int * is_start) {
	...
	printf("pqtlsa_num: %d\n", tlsa_num-1);
	char query_num[20];
	char query_url[100] = "_443._udp.ns1.";
	char query_url_postfix[20] = ".esplab.io";
...
static int load_dns_info2(struct DNS_info* dp, char* truncated_dnsmsg_out, char* dnsmsg, char * ztls_cert){
	...
	char *ztls_cert_copy;
	ztls_cert_copy = (char *)calloc(7000, sizeof(char));
	//printf("ztls_cert2: \n%s\n\n", ztls_cert);
	int k=0;
